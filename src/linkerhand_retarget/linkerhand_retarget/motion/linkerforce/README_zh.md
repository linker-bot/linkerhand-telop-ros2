...
LinkerFFG (O6/L7/L10/G20/O20/O30/O30i/R20/L25) 机械手的 ROS2 驱动模块，通过串口控制机械手，支持数据手套实时映射。
...
system:
  ...
  robotname_r: l25            # 右手机械手型号：o6 / l7 / l10 / g20 / o20 / o30 / o30i / r20 / l25
...
| O30i | 20 | 基于 G20 模板映射、使用 20 个可动 URDF 关节的 O30i 独立型号 |
| L25 | 25 | 25 自由度全功能款 |
...
LinkerFFG 会订阅阵列触觉话题，并把每个手指的触觉峰值转换为发给手套的五路力反馈值：

| 输入话题 | 方向 | 数据要求 |
|----------|------|----------|
| `/cb_left_hand_matrix_touch` | 左手 | JSON 字符串，包含五个手指的触觉矩阵 |
| `/cb_right_hand_matrix_touch` | 右手 | JSON 字符串，包含五个手指的触觉矩阵 |
...
这里的 `500` 是力反馈比例上限，对应 50% 扭矩输出，不表示 500 N。当前代码把五个手指的 `max_force` 组成 `forcelist`，通过 LinkerFFG 力反馈帧发送给手套。

按舵机 4.8 V 时扭矩 `3.5 kg·cm` 估算，若力臂按 `10 cm` 计算：

```text
输出力 = 3.5 kg·cm * 50% / 10 cm
       = 0.175 kgf
       ≈ 0.18 kgf
       ≈ 1.8 N
```

因此，当阵列传感器换算后的峰值达到 `500` 时，当前力反馈约按 50% 扭矩输出，对应末端约 `1.8 N` 的估算力。实际体感会受手套机构、摩擦、绑带松紧、供电电压和触觉矩阵标定影响，现场调试时建议先从较低触觉输入或较低比例开始验证。
...
| `robotname_r` | 右手机械手型号 | `o6`, `l7`, `l10`, `g20`, `o20`, `o30`, `o30i`, `r20`, `l25` |
