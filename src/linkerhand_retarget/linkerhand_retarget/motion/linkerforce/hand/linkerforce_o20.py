...
from ..config.o20_config import FINGER_CONFIGS, MAPPING_ORDER, ROBOT_OPOSE_RIGHT, ROBOT_OPOSE_LEFT, ROBOT_ORIGINAL_LEFT, ROBOT_ORIGINAL_RIGHT, ROBOT_FIST_LEFT, ROBOT_FIST_RIGHT, MULTI_SEGMENT_CONFIG, MULTI_SEGMENT_CONFIG_FROZEN, MOTOR_CONSTRAINTS
...
def _piecewise_motor_value(value, lower, opose, upper, motor_lower, motor_opose, motor_upper):
    if value <= opose:
        if abs(opose - lower) < 1e-9:
            return motor_opose
        ratio = (value - lower) / (opose - lower)
        return motor_lower + ratio * (motor_opose - motor_lower)

    if abs(upper - opose) < 1e-9:
        return motor_upper
    ratio = (value - opose) / (upper - opose)
    return motor_opose + ratio * (motor_upper - motor_opose)
...
class RightHand:
    ...
    def _apply_thumb_motor_calibration(self, qpos):
        for motor_idx, calibration in O20_THUMB_MOTOR_CALIBRATION.items():
            urdf_idx = self.handcore.urdfdataindex_r[motor_idx]
            lower = self.handcore.hand_lower_limits_r[urdf_idx]
            upper = self.handcore.hand_upper_limits_r[urdf_idx]
            value = _clamp(qpos[calibration["qpos_index"]], lower, upper)
            opose = self.robot_opose[calibration["robot_idx"]]
...
class LeftHand:
    ...
    def _apply_thumb_motor_calibration(self, qpos):
        for motor_idx, calibration in O20_THUMB_MOTOR_CALIBRATION.items():
            urdf_idx = self.handcore.urdfdataindex_l[motor_idx]
            lower = self.handcore.hand_lower_limits_l[urdf_idx]
            upper = self.handcore.hand_upper_limits_l[urdf_idx]
            value = _clamp(qpos[calibration["qpos_index"]], lower, upper)
            opose = self.robot_opose[calibration["robot_idx"]]
            self.g_jointpositions[motor_idx] = int(round(_piecewise_motor_value(
                value,
